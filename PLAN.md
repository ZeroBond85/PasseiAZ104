...
**Marca v7.0:** `source.png` do dono (**fonte da verdade**, revertido em 9a93f98) — derivados: letterbox `#0a0e14` `icon-192/512`, `icon-maskable-512` (arte ≤62%), `apple-touch-icon`, `favicon-32`, `header-112.png` (176×112); `hero-wide.png` (1024×650; **legado — não usado no app**); **`source-logo.webp` (1040×341, crop das margens transparentes + respiro 24px, exibição login/home/header)**. Manifest `Passei AZ-104`/`PasseiAZ104`, `education`, PT-BR, theme/bg `#0a0e14`. **Header exibe `source-logo.webp` em chip branco** (38px, `alt=""` — o nome já está no texto da marca ao lado); `header-112.png` vira legado. **Login e home exibem `source-logo.webp` sobre painel branco** (`#fff` + radius 16 + sombra — levanta o verde-escuro "by PasseiSimuladosTI", legível nos dois temas). H1 `sr-only` (logo é decorativo, `alt=""`). **Copy login:** "Do seu jeito, até a aprovação no AZ-104: simule a prova real, revise o que errou e estude no seu ritmo — em qualquer dispositivo." Formulário com hint "Você receberá um link de acesso no e-mail." **"corte 700" só na tela de Orientação** junto a 50q/100min/regras.
